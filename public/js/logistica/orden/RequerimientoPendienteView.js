                    ...
                    if (tieneTransformacion == true && cantidadItemBase == 0) {
                        ...
                        if (row.count_mapeados > 0) {
                            if (row.estado == 38 || row.estado == 39) { // estado por regularizar | estado  en pausa
                                ...
                                btnAtenderAlmacen = (array_accesos.find(element => element === 225) ? '<button type="button" class="btn btn-primary btn-xs" name="btnOpenModalAtenderConAlmacen" title="Reserva en almacén" data-id-requerimiento="' + row.id_requerimiento + '" data-codigo-requerimiento="' + row.codigo + '" data-almacen-requerimiento="' + row.almacen_requerimiento + '" '+(auth_user.id_usuario==78?'':'disabled')+'><i class="fas fa-dolly fa-sm"></i></button>' : '');
                                btnCrearOrdenCompra = (array_accesos.find(element => element === 283) ? '<button type="button" class="btn btn-warning btn-xs" name="btnCrearOrdenCompraPorRequerimiento" title="Crear Orden de Compra" data-id-requerimiento="' + row.id_requerimiento + '"  '+(auth_user.id_usuario==78?'':'disabled')+'>OC</button>' : '');
                                btnCrearOrdenServicio = (array_accesos.find(element => element === 224) ? '<button type="button" class="btn btn-danger btn-xs" name="btnCrearOrdenServicioPorRequerimiento" title="Crear Orden de Servicio" data-id-requerimiento="' + row.id_requerimiento + '" '+(auth_user.id_usuario==78?'':'disabled')+' >OS</button>' : '');
                                ...
                                // }
                                btnCrearOrdenCompra = (array_accesos.find(element => element === 283) ? '<button type="button" class="btn btn-warning btn-xs handleClickCrearOrdenCompraPorRequerimiento" name="btnCrearOrdenCompraPorRequerimiento" title="Crear Orden de Compra" data-id-requerimiento="' + row.id_requerimiento + '" '+(auth_user.id_usuario==78?'':'disabled')+' >OC</button>' : '');
