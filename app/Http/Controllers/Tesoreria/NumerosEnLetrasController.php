...
class NumerosEnLetrasController extends Controller {
	...
	public static function convertir(float $number, string $currency = '')
	{
		...
		if (empty($decimales)) {
			$valor_convertido = ucfirst(trim($converted)) . ' con ' . '00/100 ' . ($currency);
		} else {
			$valor_convertido = ucfirst(trim($converted)) . ' con ' . $decNumberStr . '/100 ' . ($currency);
		}
