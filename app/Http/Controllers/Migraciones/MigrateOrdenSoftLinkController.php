...
class MigrateOrdenSoftLinkController extends Controller
{
    ...
    public function validarOrdenSoftlink($id_orden_compra)
    {
        try {
            ...
            if ($oc->id_softlink !== null) {
                //obtiene oc softlink
                $oc_softlink = DB::connection('softtest')->table('movimien')->where('mov_id', $oc->id_softlink)->first();

                if ($oc_softlink !== null) {
                    //pregunta si fue referenciado
                    $guia_referen = DB::connection('softtest')->table('movimien')
                        ->where([
    ...
    public function migrarOrdenCompra($id_orden_compra)
    {
        try {
            ...
            $tp_cambio = DB::connection('softtest')->table('tcambio')
            ...
            if ($oc->id_softlink == null) {
                $cantidadCodigosExistentes = DB::connection('softtest')->table('movimien')->where([['num_docu', $ordenEnSoftlink['orden_softlink']], ['cod_docu', $cod_docu]])->count();
                if ($cantidadCodigosExistentes > 1) {
                    //obtiene el ultimo registro
                    $ult_mov = DB::connection('softtest')->table('movimien')
                        ->where([
                            ['num_docu', '>', $yy . '0000000'],
                            ['num_docu', '<', $yy . '9999999'],
                            ['cod_suc', '=', $cod_suc],
                    ...
                    DB::connection('softtest')->table('movimien')
                        ->where('mov_id', $ordenEnSoftlink['mov_id'])
                        ->update(['num_docu' => $num_docu]);

                    DB::connection('softtest')->table('detmov')
                        ->where('mov_id', $ordenEnSoftlink['mov_id'])
                        ->update(['num_docu' => $num_docu]);
    ...
    public function crearNuevaOrdenEnSoftlink($id_orden_compra, $oc, $detalles, $cod_suc, $cod_docu, $cuadros, $yy, $fecha, $igv, $tp_cambio)
    {
        ...
        $soc = DB::connection('softtest')->table('movimien')->where('mov_id', $mov_id)->first();
        $sdet = DB::connection('softtest')->table('detmov')->where('mov_id', $mov_id)->get();
    ...
    {
        $count = DB::connection('softtest')->table('movimien')->count();
    ...
    public function agregarOrden($mov_id, $cod_suc, $oc, $cod_docu, $num_docu, $fecha, $cod_auxi, $igv, $mon_impto, $tp_cambio, $id_orden_compra, $cuadros)
    {
        ...
        DB::connection('softtest')->table('movimien')->insert(
            [
                'mov_id' => $mov_id,
                'tipo' => '1', //Compra 
                'cod_suc' => $cod_suc,
    ...
    public function agregarDetalleOrden($det, $mov_id, $cod_prod, $cod_docu, $num_docu, $fecha, $igv, $i)
    {
        ...
        $count_det = DB::connection('softtest')->table('detmov')->count();
        //aumenta uno y completa los 10 digitos
        $mov_det_id = $this->leftZero(10, (intval($count_det) + 1));
        //Obtiene y/o crea el producto
        // $cod_prod = $this->obtenerProducto($det);

        DB::connection('softtest')->table('detmov')->insert(
            [
                'unico' => $mov_det_id,
                'mov_id' => $mov_id,
                'tipo' => '1', //Compra 
    ...
    public function actualizaStockEnTransito($oc, $cod_prod, $det, $cod_suc)
    {
        ...
        if ($stock !== null) {
            ...
            DB::connection('softtest')->table('stocks')
    ...
    {
        $vendedor = DB::connection('softtest')->table('vendedor')
            ->select('usuario')
            ->where('codvend', $oc->codvend_softlink)
            ->first();

        $count = DB::connection('softtest')->table('audita')->count();

        //Agrega registro de auditoria
        if ($accion == 'NUEVO') {
            DB::connection('softtest')->table('audita')
                ->insert([
    ...
    public function obtenerClasificacion($clasificacion)
    {
        ...
        $clasif = DB::connection('softtest')->table('soplinea')
            ->select('cod_line')
    ...
    public function obtenerCategoria($categoria, $id_categoria)
    {
        ...
        $cate = DB::connection('softtest')->table('sopsub1')
            ->select('cod_sub1')
    ...
    public function obtenerSubCategoria($subcategoria, $id_subcategoria)
    {
        ...
        $subcate = DB::connection('softtest')->table('sopsub2')
            ->select('cod_sub2')
    ...
    public function obtenerUnidadMedida($abreviatura)
    {
        ...
        $unidad = DB::connection('softtest')->table('unidades')
            ->select('cod_unid')
        ...
        if ($unidad !== null) {
            ...
            $count_unid = DB::connection('softtest')->table('unidades')->count();

            $cod_unid = $this->leftZero(3, (intval($count_unid) + 1));

            DB::connection('softtest')->table('unidades')->insert(
                [
                    'cod_unid' => $cod_unid,
    ...
    public function obtenerProveedor($nro_documento, $razon_social, $doc_tipo, $cod_di)
    {
        if ($nro_documento !== null && $nro_documento !== '') {
            $proveedor = DB::connection('softtest')->table('auxiliar')
                ->select('cod_auxi')
                ->where([
        ...
        if ($proveedor == null) {
            ...
            $mayor = DB::connection('softtest')->table('auxiliar')
                ->select('cod_auxi')
                ->where([
                    ['cod_auxi', '!=', 'TRANSF'],
                    // ['tip_auxi', '=', 'P']
            ...
            DB::connection('softtest')->table('auxiliar')->insert(
                [
                    'tip_auxi' => 'P',
                    'cod_auxi' => $cod_auxi,
                    'nom_auxi' => $razon_social,
        ...
        try {
            ...
            if (!empty($oc->id_softlink)) {
                ...
                //verifica si fue anulado
                    ...
                    DB::connection('softtest')->table('movimien')->where('mov_id', $oc->id_softlink)
                        ->update([
                            'flg_anulado' => 1,
                            'fec_anul' => new Carbon(),
                        ]);
    ...
    public function listarOrdenesSoftlinkNoVinculadas($cod_empresa, $fechaInicio, $fechaFin)
    {
        try {
            ...
            $lista = DB::connection('softtest')->table('movimien')
