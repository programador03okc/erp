...
function mostrar_crono_valorizado(){
    ...
    if (modo == 'new'){
        ...
        getCronoValorizadoEjecucion(id_presupuesto);
    }
}
    ...
    // else {
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                periodos.forEach(per => {
                    html_head += '<th>'+per.nro+'°Val. ('+per.dias+' días)</th>';
                    html_titu += '<td></td>';
                });
                html_head +='</tr>';
                ...
                lista.forEach(element => {
                    if (element.partidas !== undefined){
                        ...
                        element.partidas.forEach(partida => {
                            ...
                            periodos.forEach(per => {
                                if (fini >= per.fecha_inicio &&
                                    ...
                                    var importe = formatNumber.decimal(valor,'',-2);
                                    html_periodos += '<td class="right">'+importe+'</td>';
                                    if (per.total !== undefined){
                                        per.total = parseFloat(per.total) + parseFloat(valor);
                                    } else {
                                        per.total = parseFloat(valor);
                                    }
                                    if (partida.fecha_fin > per.fecha_fin){
                                        fini = per.fecha_fin;
                                    }
                        ...
                        '<td>'+element.dias+'</td>'+
                        '<td class="right">'+formatNumber.decimal(imp,'',-2)+'</td>'+
                        html_periodos+
                        '</tr>';
                    }
                });
                ...
                var html_foot = '<tr><td></td>'+
                '<td></td>'+
                '<td></td>'+
                '<th class="right">Totales</th>';
            
                periodos.forEach(per => {
                    html_foot += '<th class="right">'+formatNumber.decimal(per.total,'',-2)+'</th>';
                });
                html_foot +='</tr>';
...
function getCronoValorizadoEjecucion(id_presupuesto){
    $.ajax({
        type: 'GET',
        url: 'listar_cronoval_ejecucion/'+id_presupuesto,
        dataType: 'JSON',
        success: function(response){
            console.log(response);
...
function save_cronoval(){
    ...
    if (msj.length > 0){
        ...
        $.ajax({
            type: 'POST',
            url: 'guardar_cronoval_ejecucion',
            data: data,
            dataType: 'JSON',
            success: function(response){
                console.log(response);
