                    ...
                    if (tieneTransformacion == true && cantidadItemBase == 0) {
                        ...
                        if (row.count_mapeados > 0) {
                            if (row.estado == 38 || row.estado == 39) { // estado por regularizar | estado  en pausa
                                ...
                                // }
                                btnCrearOrdenCompra = (array_accesos.find(element => element === 283) ? '<button type="button" class="btn btn-warning btn-xs handleClickCrearOrdenCompraPorRequerimiento" name="btnCrearOrdenCompraPorRequerimiento" title="Crear Orden de Compra" data-id-requerimiento="' + row.id_requerimiento + '" disabled >OC</button>' : '');
